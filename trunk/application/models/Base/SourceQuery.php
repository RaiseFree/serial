...
use \Source as ChildSource;
use \SourceQuery as ChildSourceQuery;
use \Exception;
use \PDO;
use Map\SourceTableMap;
 ...
 * @method     ChildSourceQuery orderBySerialId($order = Criteria::ASC) Order by the serial_id column
 * @method     ChildSourceQuery orderByName($order = Criteria::ASC) Order by the name column
 * @method     ChildSourceQuery orderByUrl($order = Criteria::ASC) Order by the url column
 ...
 * @method     ChildSourceQuery groupBySerialId() Group by the serial_id column
 * @method     ChildSourceQuery groupByName() Group by the name column
 * @method     ChildSourceQuery groupByUrl() Group by the url column
 ...
 * @method     ChildSource findOneBySerialId(int $serial_id) Return the first ChildSource filtered by the serial_id column
 * @method     ChildSource findOneByName(string $name) Return the first ChildSource filtered by the name column
 * @method     ChildSource findOneByUrl(string $url) Return the first ChildSource filtered by the url column
 ...
 * @method     ChildSource requireOneBySerialId(int $serial_id) Return the first ChildSource filtered by the serial_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildSource requireOneByName(string $name) Return the first ChildSource filtered by the name column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildSource requireOneByUrl(string $url) Return the first ChildSource filtered by the url column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildSource[]|ObjectCollection findBySerialId(int $serial_id) Return ChildSource objects filtered by the serial_id column
 * @method     ChildSource[]|ObjectCollection findByName(string $name) Return ChildSource objects filtered by the name column
 * @method     ChildSource[]|ObjectCollection findByUrl(string $url) Return ChildSource objects filtered by the url column
...
abstract class SourceQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Base\SourceQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'serial_db', $modelName = '\\Source', $modelAlias = null)
    {
     ...
     * </code>
     *
     * @param array[$id, $serial_id] $key Primary key to use for the query
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = SourceTableMap::getInstanceFromPool(serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])])))) && !$this->formatter) {
    ...
    {
        $sql = 'SELECT id, serial_id, name, url, created_at, updated_at FROM source WHERE id = :p0 AND serial_id = :p1';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj = new ChildSource();
            $obj->hydrate($row);
            SourceTableMap::addInstanceToPool($obj, serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]));
        }
        $stmt->closeCursor();
    ...
    {
        $this->addUsingAlias(SourceTableMap::COL_ID, $key[0], Criteria::EQUAL);
        $this->addUsingAlias(SourceTableMap::COL_SERIAL_ID, $key[1], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            $cton0 = $this->getNewCriterion(SourceTableMap::COL_ID, $key[0], Criteria::EQUAL);
            $cton1 = $this->getNewCriterion(SourceTableMap::COL_SERIAL_ID, $key[1], Criteria::EQUAL);
            $cton0->addAnd($cton1);
            $this->addOr($cton0);
        }
    ...
    public function filterBySerialId($serialId = null, $comparison = null)
    {
        if (is_array($serialId)) {
            ...
            if (isset($serialId['min'])) {
                $this->addUsingAlias(SourceTableMap::COL_SERIAL_ID, $serialId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($serialId['max'])) {
                $this->addUsingAlias(SourceTableMap::COL_SERIAL_ID, $serialId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(SourceTableMap::COL_SERIAL_ID, $serialId, $comparison);
    }
    ...
    public function filterByName($name = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(SourceTableMap::COL_NAME, $name, $comparison);
    }
    ...
    public function filterByUrl($url = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(SourceTableMap::COL_URL, $url, $comparison);
    }
    ...
    {
        if ($source) {
            $this->addCond('pruneCond0', $this->getAliasedColName(SourceTableMap::COL_ID), $source->getId(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond1', $this->getAliasedColName(SourceTableMap::COL_SERIAL_ID), $source->getSerialId(), Criteria::NOT_EQUAL);
