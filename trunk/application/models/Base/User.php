...
abstract class User implements ActiveRecordInterface
{
    ...
    /**
     * The value for the login_at field.
     * 最后登录时间
     * @var        \DateTime
     */
    protected $login_at;
    ...
    /**
     * Get the [optionally formatted] temporal [login_at] column value.
     * 最后登录时间
     *
     ...
     */
    public function getLoginAt($format = NULL)
    {
        if ($format === null) {
            return $this->login_at;
        } else {
            return $this->login_at instanceof \DateTime ? $this->login_at->format($format) : null;
        }
    }
    ...
    /**
     * Sets the value of [login_at] column to a normalized version of the date/time value specified.
     * 最后登录时间
     * @param  mixed $v string, integer (timestamp), or \DateTime value.
     *               Empty strings are treated as NULL.
     * @return $this|\User The current object (for fluent API support)
     */
    public function setLoginAt($v)
    {
        $dt = PropelDateTime::newInstance($v, null, 'DateTime');
        if ($this->login_at !== null || $dt !== null) {
            if ($this->login_at === null || $dt === null || $dt->format("Y-m-d H:i:s") !== $this->login_at->format("Y-m-d H:i:s")) {
                $this->login_at = $dt === null ? null : clone $dt;
                $this->modifiedColumns[UserTableMap::COL_LOGIN_AT] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : UserTableMap::translateFieldName('LoginAt', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->login_at = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_LOGIN_AT)) {
            $modifiedColumns[':p' . $index++]  = 'login_at';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'email':
                        ...
                        break;
                    case 'login_at':
                        $stmt->bindValue($identifier, $this->login_at ? $this->login_at->format("Y-m-d H:i:s") : null, PDO::PARAM_STR);
                        break;
                    case 'status':
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_LOGIN_AT)) {
            $criteria->add(UserTableMap::COL_LOGIN_AT, $this->login_at);
        }
        if ($this->isColumnModified(UserTableMap::COL_STATUS)) {
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setLoginAt($this->getLoginAt());
