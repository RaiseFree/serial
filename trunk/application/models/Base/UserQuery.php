 ...
 * @method     ChildUserQuery orderByLoginAt($order = Criteria::ASC) Order by the login_at column
 ...
 * @method     ChildUserQuery groupByLoginAt() Group by the login_at column
 ...
 * @method     ChildUser findOneByLoginAt(string $login_at) Return the first ChildUser filtered by the login_at column
 ...
 * @method     ChildUser requireOneByLoginAt(string $login_at) Return the first ChildUser filtered by the login_at column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildUser[]|ObjectCollection findByLoginAt(string $login_at) Return ChildUser objects filtered by the login_at column
...
abstract class UserQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'serial_db', $modelName = '\\User', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, name, password, email, login_at, status, created_at, updated_at FROM user WHERE id = :p0';
        try {
    ...
    /**
     * Filter the query on the login_at column
     *
     * Example usage:
     * <code>
     * $query->filterByLoginAt('2011-03-14'); // WHERE login_at = '2011-03-14'
     * $query->filterByLoginAt('now'); // WHERE login_at = '2011-03-14'
     * $query->filterByLoginAt(array('max' => 'yesterday')); // WHERE login_at > '2011-03-13'
     * </code>
     *
     * @param     mixed $loginAt The value to use as filter.
     ...
     * @return $this|ChildUserQuery The current query, for fluid interface
     */
    public function filterByLoginAt($loginAt = null, $comparison = null)
    {
        if (is_array($loginAt)) {
            $useMinMax = false;
            if (isset($loginAt['min'])) {
                $this->addUsingAlias(UserTableMap::COL_LOGIN_AT, $loginAt['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($loginAt['max'])) {
                $this->addUsingAlias(UserTableMap::COL_LOGIN_AT, $loginAt['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UserTableMap::COL_LOGIN_AT, $loginAt, $comparison);
    }
